...
public class SortArrayUsingSelectionSORT {
	...
	public static void selectionSort(int arr[]) {
		...
		{                            //becaues last element of array get automatically sort
			...
			int min=i;	//let consider now our 0th index element is minimum hence storing index of that in min
			...
			for(int j=i+1;j<n;j++)	//to compare with i, j will start from next num i.e. i+1
			{
				...
				{						// just replace min  to 2nd num/minimum no's index  
					...
					min=j;		// min is replace by smallest element index in array i.e. j 
				}
			}
	...
	public static void main(String[] args) {
		...
		int N=sc.nextInt();
		int arr[]=new int[N];		//my_array arr[] passing N as size of array and new is keyword
											//that allocates the memory to us
		for(int m=0;m<=N-1;m++)
		{
			arr[m]=sc.nextInt();
		}
		selectionSort(arr);		//calling fucntion and passing array 
